...
from utils.cross_attn_encoder import BertConfig, MambaLayer, MambaLayer_singlelayer
...
class rob_hub_mamba(nn.Module):
    def __init__(self, config):
        ...
        self.Mambalayers = MambaLayer_singlelayer(Bert_config)
        # fused method V2
    ...
    def forward(self, text_inputs, text_mask, audio_inputs, audio_mask):
        ...
        text_inputs, audio_inputs = self.Mambalayers(text_inputs, text_attn_mask, audio_inputs, audio_attn_mask)

        #text_self_att_outputs_cls = text_inputs[:, 0, :]  # [batch_size, cls, 768]
       # audio_self_att_outputs_cls = audio_inputs[:, 0, :]  # [batch_size, cls, 768]
        ...
        if self.version == 'v1':
            ...
            fused_hidden_states = torch.cat((text_mixed_features, audio_mixed_features),
                                            dim=1)  # Shape is [batch_size, 768*2]
        elif self.version == 'v4':  # cross-attention - self-attention - ffn - multihead-attention- fn- output
            fused_features = self.bi_gated_model(text_inputs[:, 0, :], audio_inputs[:, 0, :])  # shape is [batch_size, 768]
        else:
            # concatenate original features with fused features
            text_concat_features = torch.cat((T_hidden_states, text_inputs[:, 1:, :]),
                                             dim=2)  # Shape is [batch_size, text_length, 768*2]
            audio_concat_features = torch.cat((A_hidden_states, audio_inputs[:, 1:, :]),
                                              dim=2)  # Shape is [batch_size, audio_length, 768*2]
            text_concat_features, text_attn_mask = self.prepend_cls(text_concat_features, text_mask,
                                                                    'text_mixed')  # add cls token
            audio_concat_features, audio_attn_mask = self.prepend_cls(audio_concat_features, audio_mask_new,
